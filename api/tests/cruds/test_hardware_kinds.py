...
def test_update_hardware_kind(create_hardware_kind, hardware_kind_repository, actor):
    hw_kind = create_hardware_kind(kind="Sensor Inicial")
...
def test_delete_hardware_kind(create_hardware_kind, hardware_kind_repository):
    hw_kind = create_hardware_kind(kind="Sensor Deletar")
    hardware_kind_repository.delete(hw_kind.id)
    
    deleted_hw = hardware_kind_repository.get(hw_kind.id)
    assert deleted_hw is None

def test_get_list_hardware_kinds(create_hardware_kind, hardware_kind_repository):
    # cria alguns hardware kinds
    create_hardware_kind(kind="Sensor 1")
    create_hardware_kind(kind="Sensor 2", is_active=False)
